...
function Client(server, nick, opt) {
    ...
    self.opt = {
        ...
        password: null,
        userName: 'IRISChat',
        realName: 'IRISChat - nodeJS IRC client',
        port: 6667,
        localAddress: null,
        debug: false,
        showErrors: false,
        ...
        switch (message.command) {
           /* case 'rpl_welcome':
            ...
            case 'rpl_namreply':
                ...
                jsonfile.writeFile('../userdata.json', users, function (err) {
                    console.error(err);
                    /*console.log(users);*/
                    lusers = users;
            ...
            /*case 'rpl_away':
                ...
                /* WHOIS REPLIY USER, CHANNEL, SERVER, IDLE */
            case 'rpl_whoisuser':
            ...
            case '330': // rpl_whoisaccount?
                ...
                /*console.log('whoisaccount: ', message)*/
                break;
            case 'rpl_endofwhois':
            ...
            case 'rpl_listend':
                ...
                jsonfile.writeFile('channeldata.json', self.channellist, function (err) {
                    console.error(err)
                });
...
Client.prototype.topic = function(channel, newTopic) {
    if (newTopic) {
        this.send('TOPIC', channel, newTopic);
    }
};
...
Client.prototype.names = function(channel) {
    this.send('NAMES', channel);
    /*if(lusers){
    console.log("irc"+ lusers.toString());
    return lusers;}*/
};
